...
# STEP 5  Use KNN to classify new data.
...
N_CLUSTERS = 4
KNN_K = 3  # ปรับลด K ของ KNN ให้เหมาะสมกับจำนวนข้อมูลที่น้อยลง
...
def main():
    ...
    for k in k_values:
        ...
        print(
            f"   k = {k}  ->  inertia = {km.inertia_:8.1f}   silhouette = {sil:.3f}"
        )
    ...
    #  scatter plot ใช้คอลัมน์แรกๆ ของข้อมูล
    visualize.plot_clusters(
        X_raw[:, [0, 1]], labels, OUT_DIR / "02_clusters.png"
    )
    ...
    title(f"STEP 5 : use KNN detect groups (k = {KNN_K})")

    # ใช้สัดส่วน 80% สำหรับ Known Data และ 20% สำหรับ New Data
    ...
    print(
        f"KNN detect groups correctly compared to K-Means : {accuracy * 100:.1f} %"
    )
    ...
    result.to_csv(
        OUT_DIR / "clustered_results.csv", index=False, encoding="utf-8-sig"
    )
